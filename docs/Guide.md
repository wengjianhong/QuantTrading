...
- **启动编排**：进程入口启动时出站 `GetEngineConfig`，再 `IEngine::Init(EngineConfig)`；**不**订阅运行时推送，配置变更须停引擎后重 Init/Start
...
|进程入口 ↔ config-service|gRPC + Protobuf|出站 `GetEngineConfig` 后 `IEngine::Init`；引擎不持配置 gRPC；无运行时 Subscribe|
...
- **启动编排（config）**：进程入口冷启动 `GetEngineConfig` 后 `IEngine::Init`；策略经 `AddStrategy(config, plugin_so_path)` 登记。**不**做运行时 Subscribe/Watch。变更须停引擎后重 Init/Start
...
- 引擎业务配置由进程入口拉取后经 `IEngine::Init` 注入；策略插件经 `AddStrategy(config, so路径)` 加载；**交易凭证**经 account-service 单独管理（《架构》§2.6）；禁止外部直接修改交易引擎内存；运行中不接受配置热推
...
| config 启动拉取（GetEngineConfig + Init） | MVP | ✅ 进程入口拉配置；引擎无配置 gRPC；无运行时 Watch |
