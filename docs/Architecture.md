...
- **低延迟**：**本地决策链路**（A 段，§2.1.1：行情出队至 `OrderIntent` 入队）P99 < 50 微秒；账户风控、订单落盘、出站报单和交易所 RTT 分开计时；
- **可恢复**：当前阶段支持进程自动重启、订单日志恢复和交易所对账；暂不承诺自动主备、跨机房切换及其 RTO/RPO；
...
- **低延迟优化**：A 段（§2.1.1）全程进程内内存交互；订单日志在独立 J 段提交；普通快照、指标和旁路上报异步处理。
- **可恢复性**：订单发往交易所前必须先提交订单主日志；重启后先恢复、再对账、最后恢复策略。主备与跨机房灾备后续单独设计。
...
| **E. 账户风控** | **否** | `OrderIntent` 出队 → `account-risk-service` 原子预占。由独立准入线程执行，不占用 Lane-M；Production/Institutional 强制启用，Lite 可选 |
| **J. 订单落盘** | **否，单独 SLA** | 订单主日志提交 `Prepared` → 订单状态协调器应用内存状态。未提交的订单不得交给 EMS |
| **C. 出站发单** | **单独 SLA** | EMS 出队 → 执行适配器 → 交易所/券商 |
| **B. 异步副本** | **否** | 内存快照、历史副本、报表和普通运行日志；失败不影响订单主记录 |
| **D. 旁路上报** | **否** | `log_client`/`monitor_client` 等单向 fire-and-forget 上报，不等待响应 |

- **A 段 P99 < 50 微秒**：只表示本地策略和风控决策速度，不代表完整报单延迟。
- **完整报单延迟**：必须同时报告 A、E、J、C，以及 A → [E] → J → C 总耗时。
- **吞吐指标**：同时报告单实例吞吐和多实例聚合吞吐，不能只用增加实例数满足总量目标。
- **发布门槛**：每个部署档位必须在上线前填写 E/J/C、完整链路、最大队列驻留时间和恢复时间的数值目标；未填写或未通过压测不得发布。

#### 2.1.2 基准压测场景（单进程）

| 场景 | 延迟目标（P99） | 说明 |
|---|---|---|
| 空策略 passthrough | < 20μs | 只验证 Lane-M、策略调度和 `OrderIntent` 入队 |
| 1 策略 + CMS + 全量本地风控 | < 50μs | MVP 默认验收；A 段统一止于 `OrderIntent` 入队 |
| N 策略并行 + M 品种 | 单独指标 | 按客户部署压测报告交付 |
| A → [E] → J → C 完整链路 | 按部署档位填写 | 必须覆盖账户风控、日志提交、EMS 排队和适配器调用 |

MVP 在单机房、生产同等级硬件下采用以下基线；客户部署可提出更严格目标：

| 指标 | P99 | P99.9 | 说明 |
|---|---:|---:|---|
| A 段 | < 50μs | < 100μs | 不含 E/J/C |
| E 段 | < 2ms | < 5ms | 同机房账户风控事务 |
| J 段 | < 2ms | < 5ms | Production 含异机副本确认 |
| C 段本地处理 | < 100μs | < 250μs | EMS 出队至请求交给柜台 SDK，不含交易所 RTT |
| A → E → J → C | < 5ms | < 10ms | 不含交易所 RTT |
| Lane-R 队列驻留 | < 100μs | < 500μs | 回报洪峰单独压测 |
...
独立的订单准入线程读取 `OrderIntent`。Production/Institutional 必须先执行 E 段预占；Lite 可按配置跳过 E 段。J 段将订单写成 `Prepared` 并提交订单主日志，提交成功后才允许进入 EMS。

`ReserveOrder` 返回三种结果：

- **Approved**：进入 J 段；
- **Rejected**：记录拒绝原因并归还本地预算；
- **Unknown**：表示可能已预占但响应丢失。此时不得发单，也不得直接当作拒绝；必须使用同一 `order_id` 查询预占结果，或进入持久化补偿流程。

J 段失败时冻结对应账户的新单，并启动预占查询/释放流程，不能只告警。

这样做的目的很简单：行情线程不被远程风控拖慢，同时保证订单在发往交易所前已有可恢复记录。

### 2.2 热路径与非热路径职责

- **分段与时序以 §2.1、§7.2 为准**：A 段在 Lane-M 执行策略、CMS 与实例风控；订单准入线程执行 E 段；J 段日志提交成功后，订单状态协调器应用 OMS/Account/PMS 状态并交给 EMS；C 段负责出站。订单/成交回报也先提交订单主日志，再更新内存状态和通知策略。
...
所有核心模块运行在同一进程内。Lane-M 处理行情，Lane-R 接收订单和成交回报；订单状态协调器统一修改 OMS、Account 和 PMS，避免两个线程同时改同一份交易状态。
...
|---|---|---|---|---|
|**事件总线（EventLanes）**|分发 Tick/Bar 与订单/成交回报|只负责事件排队；不做业务校验、账簿更新或持久化|否，核心基础设施|Lane-M 与 Lane-R 各自使用有界队列；队列满时按 §7.1 处理|
|**行情标准化（QuoteNormalizer）**|校验并统一厂商行情，发布到 Lane-M|不做策略调度、风控或二次排队|模块固定；`QuoteApi` 可替换|持有 `QuoteApi`，订阅实例品种并集；断线重连/切源在此或启动编排层执行|
|**交易标准化（TraderNormalizer）**|统一订单/成交回报，发布到 Lane-R|不直接调用 OMS，也不发送订单|模块固定；`TraderSpi` 适配器可替换|映射订单状态、数量和错误码；OMS 更新由订单状态协调器负责|
...
|**订单状态协调器 + OMS**|统一处理新订单、撤单、订单回报和成交回报|是 OMS、Account、PMS 的单写者；不执行策略、磁盘 I/O 或厂商协议|核心流程固定；订单类型可扩展|向独立日志线程提交事件；按每订单顺序等待 commit 回执，不因单次日志 I/O 阻塞全部回报|
...
- **调用顺序**：策略信号 → **CMS**（合规硬规则）→ **RiskManager**（实例实时风险）→ `OrderIntent` → E 段账户预占（Production/Institutional 强制）→ J 段订单落盘 → EMS；任一环节失败都终止并记录原因。
...
- **释放方式**：Lane-R 不同步等待 `ReleaseOrder`。订单主日志提交回报事件时同时产生 outbox（待发送任务日志），再由后台线程重试释放或结算，避免风险服务变慢时堵住回报通道。
...
- **状态更新顺序**：规范化回报先提交订单主日志；commit 成功后，订单状态协调器更新 OMS、Account、PMS，再将不可变回报事件放入策略 mailbox。
- **跨 Lane 读写**：Lane-M 只读取带版本的只读快照，不直接修改 OMS、Account 或 PMS。
- **发单 / 持久化**：完整路径见 **§7.2**（A → [E] → J → C）。
...
| 当前状态 | 事件 | 新状态/动作 |
|---|---|---|
| `Prepared` | 发送前 | 先持久化 `SendPending`，再调用柜台 |
| `SendPending` | 明确确认请求未提交，且错误可重试 | `SendRetryPending`；使用同一 `order_id` 有界重试，保留预算和预占 |
| `SendPending/SendRetryPending` | 明确确认请求未提交，且错误不可重试或重试耗尽 | `SendFailed`；记录原因并异步释放账户预占 |
...
1. 将规范化事件和幂等键提交到**订单主日志**；
2. 订单状态协调器按日志序号更新 OMS、Account、PMS，并记录应用水位；
3. 再通知策略，并通过 outbox 复制到风控、审计和历史服务；
4. 恢复时从应用水位之后重放，订单主日志是唯一恢复事实源。

发送外部请求无法与本地日志组成一个事务，因此恢复时残留的 `SendPending` 一律按 `SendUnknown` 处理。
...
4. **A 段热路径**（§2.1.1）禁止同步远程调用和磁盘 I/O；E/J/C 均由独立线程执行，不占用 Lane-M
...
|**账户风控服务（account-risk-service）**|执行账户资金、保证金、总/净敞口等硬限制|账户硬风控账簿与预占记录权威来源；不管理凭证或策略配置|引擎 E 段调用 `ReserveOrder`；释放和结算经持久化 outbox 异步重试|按账户单写；预占和账簿更新在同一事务；支持幂等、部分结算、恢复查询和对账|
...
|**日志服务（log-service）**|接收、检索和归档运行日志与交易流水|保存日志副本，不承担订单主日志或账户账簿职责|引擎经 D 段异步 client 投递；存储后端可替换|分级、批量写入、保留期、哈希校验和按租户检索；P0 审计依赖本地 spool 防缺口|
|**监控服务（observability-service）**|采集指标、告警和性能分析数据|不参与订单准入；只消费监控副本|引擎经 monitor client 上报；告警通道可插拔|关注 A/E/J/C 延迟、队列深度、拒绝率、账簿偏差与行情切换；支持告警分级|
...
6. **账户级硬风控**：只有 `ReserveOrder` 属于 E 段同步准入；`ReleaseOrder`/结算由持久化 outbox 异步重试。明确拒绝才按拒单处理；超时或连接中断进入 `Unknown` 查询流程，禁止降级为本地账户快照放行
...
适配器不包含策略、风控、OMS 或账户业务。行情链路为“适配器 → QuoteNormalizer → Lane-M”，交易回报链路为“适配器 → TraderNormalizer → Lane-R → 订单状态协调器”；出站订单由 EMS 经交易 Api 发送。SDK 接口、继承关系、目录和实现示例见《[Guide.md](Guide.md)》§5.1。
...
- **边界**：Api 适配器承接 EMS 的出站请求，Spi 适配器将柜台回报交给 `TraderNormalizer` → Lane-R → 订单状态协调器。适配器不裁决策略、合规或风控。
...
- **交易引擎内部**：Lane-M/Lane-R 使用独立有界队列；订单状态协调器是 OMS、Account、PMS 的单写者；策略通过串行 mailbox 接收事件
...
- **引擎 ↔ 安全控制能力（紧急操作）**：
  - Production/Institutional 至少配置两个独立 endpoint；安全命令先持久化，引擎重连后可继续领取未完成命令
...
- **引擎 ↔ account-risk-service（E 段，账户级硬风控）**：
  - 发单前：A 段完成 CMS 与实例预算校验后，以 `order_id` 幂等调用 `ReserveOrder(account_id, OrderIntent, release_id)`；仅 `Approved` 才进入 J 段
  ...
  - 回报后：规范化事件先提交订单主日志，订单状态协调器再更新本地状态；结算/释放任务由日志事务产生的 outbox 异步重试，不阻塞 Lane-R
  - 部分成交：已成交部分转为实际占用，剩余挂单继续保留预占；只有终态才能释放剩余预占
  - TTL：到期后进入待对账状态，不直接释放额度
  - 恢复：引擎重启后查询未终结预占，并与 OMS、订单日志和柜台订单对账
...
| `ReleaseOrder` / `SettleOrder` | Unary | outbox worker → account-risk-service | 后台幂等释放或结算，不属于 E 段 |
...
| 数据级别 | 示例 | 远程上报不可用时的行为 |
|---|---|---|
| **P0 交易与审计事实** | 准入结果、发单/撤单、全部订单/成交回报、配置版本、安全控制命令 | 订单类事实写入订单主日志；其他事实写入审计日志。远程不可用时从各自主日志重放，不能丢弃 |
...
- A 段只产生内存 `OrderIntent`，不执行磁盘 I/O。准入线程一旦开始处理该 Intent，就必须在订单主日志中留下 `Prepared`、`RiskRejected` 或 `ReserveUnknown` 之一。
- **订单主日志是订单恢复的唯一事实源**；P0 spool 只是远程审计投递的 outbox 副本，不再重复维护另一套订单事实。
...
| 队列 | 满队列时怎么做 |
|---|---|
| **Lane-M 行情队列** | 可合并同品种的全量快照或丢弃已过期 Tick；如果策略依赖完整逐笔数据，则暂停该品种新开仓并触发补数 |
| **Lane-R 回报队列** | 不允许静默丢弃；使用预留应急容量或独立持久化 inbox。仍无法接收时冻结新单，并按适配器能力从柜台查询补齐 |
...
| **订单主日志 / 审计日志** | 接近容量上限时冻结新单；保留回报、撤单和恢复写入空间 |

每个部署档位必须提交容量预算表，至少包含：行情峰值、订单/回报峰值及持续时间、各队列容量与高水位、日志增长速度、磁盘保留空间、支撑服务中断容忍时间、恢复后的追赶速度和积压清空时间。容量预算未完成不得进入性能验收。
...
|---|---|---|
| **Lane-M** | 行情适配器 → `QuoteNormalizer` | `StrategyEngine` → CMS → 实例 Risk → `OrderIntent` |
| **Lane-R** | 交易适配器 → `TraderNormalizer` | 订单状态协调器 → OMS/Account/PMS → 策略 mailbox |
...
外部行情 → 适配器 → QuoteNormalizer → Lane-M
  ...
  → 订单主日志提交 Prepared → 订单状态协调器应用                 [J 段]
  → EMS 出队 → 适配器 → 交易所                                  [C 段]
  ╲ 内存快照 / 历史副本 / 指标                                  [B/D 段，异步]
```
...
| E 段明确拒绝 | `RiskRejected` 落日志 | 归还 | 未发生 | 结束 |
| E 段结果未知 | `ReserveUnknown` 落日志 | 暂不归还 | 查询确认 | 未确认前不得发单 |
| E 成功、J 失败 | 无可发送订单 | 暂不归还 | 保留并查询 | 冻结账户新单，确认柜台无单后幂等释放 |
| J 成功、EMS 暂不可用 | `Prepared/SendPending` | 保留 | 保留 | 持续重试或恢复后继续发送 |
| 柜台明确确认请求未提交 | `SendRetryPending` 或 `SendFailed` | 重试期间保留；终止后归还 | 重试期间保留；终止后异步释放 | 仅使用同一 `order_id` 有界重试；错误不可重试或次数耗尽则终止 |
...
|交互场景|推荐协议|核心理由|
|---|---|---|
|交易引擎内部|内存结构体 + 有界队列|Lane-M/Lane-R 入口隔离；订单状态由单写者统一更新|
...
|引擎 ↔ account-risk-service（账户硬风控）|gRPC + Protobuf|准入线程同步 `ReserveOrder`；结算/释放由持久化 outbox 异步重试|
...
```text
BOOTSTRAP → FENCED → REPLAYED → BROKER_SYNCED
  → RISK_SYNCED → MARKET_HEALTHY → READY
  → FREEZE → DRAIN → STOPPED
```

- 只有 `READY` 可以接受新单。
- `FREEZE/DRAIN` 允许撤单和回报，禁止新单；停机前必须处理未完成日志、outbox 和撤单确认。
- 存在 `SendUnknown`、孤儿预占、配置过期、行情不健康或对账差异时，不得进入 `READY`；人工豁免必须审批并审计。
- **进程级故障**：自动重启后依次执行：取得新 `engine_epoch` → 恢复订单主日志和 outbox → 登录柜台 → 查询未终结订单和成交 → 对账账户预占 → 检查行情健康 → 人工或受控恢复策略。
- **恢复验收目标**：在柜台、日志副本和账户风控服务可用的前提下，MVP 从进程退出到 `READY` 不超过 15 分钟；该指标不等同于主备自动切换 RTO。

- **节点级故障**：通过部署/运维系统重建同一 `engine_id` 实例。Production/Institutional 的订单主日志必须有异机副本；只保存在故障节点本地的数据不能作为唯一恢复依据。

- **行情源故障**：检测到断流、序号缺口或陈旧行情时，先暂停受影响品种的新开仓。切换数据源后完成快照恢复和盘口校验，再恢复交易；本地缓冲只能缓解处理拥塞，不能补回断流期间未收到的数据。

| 故障 | 允许动作 | 恢复条件 |
|---|---|---|
| 订单日志不可写、损坏或空间不足 | 冻结新单；继续回报和撤单写入保留空间 | 日志恢复并通过完整性检查 |
...
| 柜台查询不可用 | 保持 `SendUnknown` 和预占，不恢复策略 | 未终结订单与成交查询完成 |
| 时钟偏差超限 | 冻结新单 | 时钟恢复并重新校验租约/配置 |
| 策略卡死或超时 | 熔断该策略并冻结其新单 | 人工确认或受控重启 |
| outbox 毒消息 | 隔离单条消息并告警，不阻塞同分区后续消息 | 修复后按事件 ID 重放 |

### 8.3 数据一致性保障

- **订单主日志提交语义**：
  - Lite：本机持久化提交，可接受节点故障时非零 RPO，但必须在部署说明中给出数值；
  - Production/Institutional：本机持久化成功并收到异机副本确认后才算 J 段提交，已提交记录的目标 RPO 为 0；
  - 日志记录必须带序号、长度和校验信息，启动时可识别并截断不完整尾记录；
  - 内存快照只用于加快恢复，不能代替订单主日志。

- **核心交易数据**：重启后必须与交易所/清算对账，以外部订单和成交为最终事实；本地日志保存订单意图、风控决定及本地/柜台 ID 映射；

- **订单幂等性**：`order_id` 必须包含租户、引擎和 epoch 信息并跨重启唯一；同时持久化本地 ID、柜台客户端 ID、交易所订单号和交易日的映射。柜台返回未知结果时，先查询再决定是否重发；
...
|交易通道故障|订单进入 SendUnknown，先查询再决定是否重发|持久化本地 ID 与柜台 ID 映射|
|策略异常|异常捕获 + 策略熔断 + 资源配额；发布前扫描|故障限定于单实例配置内品种 |
|交易引擎崩溃|持久化订单日志 + 快照 + 自动重启|恢复后先对账再启策略；使用 engine epoch 防双写|
|配置更新异常|版本、签名、有效期、灰度和回滚|快照过期后冻结新单；品种归属变更需维护窗口|
|数据不一致|完整订单状态机 + 恢复对账 + 账户级原子预占|部分成交按量结算；TTL 到期先对账|
|人为干预|数据面封闭 + 控制面经 config 审计|关键配置双人复核|
|回报被行情阻塞|Lane-R 独立入口；订单状态协调器优先处理回报|回报不可丢；策略通过串行 mailbox 接收|
...
|阶段|建设重点|交付目标|分片/行情|
|---|---|---|---|
|**一期（MVP）**|订单状态机、持久化订单日志、单写订单状态、config + account + account-risk + 安全控制 + observability|单租户、单账户单 active shard（dry-run → 受控实盘）|静态分片、账户预占、engine epoch、恢复对账|
