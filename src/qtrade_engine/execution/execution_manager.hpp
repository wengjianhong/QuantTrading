...
class ExecutionManager final : public ExecutionApi {
 public:
  ...
  void SetOrderApi(orders::OrderApi* order_api);

  /// @brief 绑定账户硬风控模块间接口（发送失败时释放预占）
  /// @param account_risk 可空；空则发送失败不释放
  void SetAccountRiskApi(account_risk::AccountRiskApi* account_risk);
 ...
 private:
  ...
  /// OMS 稳定接口；未设置时跳过状态回写
  orders::OrderApi* order_api_ = nullptr;
  /// 账户硬风控接口；未设置时跳过失败释放
  account_risk::AccountRiskApi* account_risk_ = nullptr;
  /// 待发送工作项队列；撤单从队头插入
