...
std::uint64_t InstanceRiskManager::Version() const {
  std::lock_guard lock(mutex_);
  return limits_.version;
}

ErrorCode InstanceRiskManager::CheckOrder(const qtrade::sdk::trader::OrderRequest& request) const {
  // 1. 基础字段合法性
