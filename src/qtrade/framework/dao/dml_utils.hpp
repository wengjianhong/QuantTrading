...
/// @param connection 由 Repository 持有的 IConnection 指针；nullptr 表示清除
void SetConnection(const std::string& database_name, cpputils::database::IConnection* connection);

/// @brief 按数据库名获取已注册连接
/// @param database_name 逻辑数据库名
/// @return 当前连接指针；未注册时返回 nullptr
[[nodiscard]] cpputils::database::IConnection* GetConnection(const std::string& database_name);
...
[[nodiscard]] Result<std::int64_t> InsertRow(const std::string& database_name,
...
[[nodiscard]] Result<std::int64_t> DeleteRows(const std::string& database_name,
                                              const std::string& table,
...
/// @return 成功时 data 为受影响行数
[[nodiscard]] Result<std::int64_t> UpdateRows(const std::string& database_name,
                                              const std::string& table,
                                              const KeyValues& values,
                                              const KeyValues& where_values);
...
/// @return 成功时 data 为行数
[[nodiscard]] Result<std::int64_t> CountRows(const std::string& database_name,
                                             const std::string& table,
                                             const KeyValues& where_values);
...
[[nodiscard]] Result<std::unique_ptr<cpputils::database::IResultSet>> SelectRows(const std::string& database_name,
                                                                                 const std::string& table,
                                                                                 const KeyValues& where_values);
...
/// @param table 表名
/// @return 成功时 data 为受影响行数
[[nodiscard]] Result<std::int64_t> TruncateRows(const std::string& database_name, const std::string& table);
