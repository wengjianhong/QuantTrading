...
[[nodiscard]] bool ProtoToJson(const MessageT& message,
                               std::string& out,
                               const ProtoJsonOptions& options = {},
                               const std::string_view log_tag = "ProtoJson") {
  ...
  if (!status.ok()) {
    spdlog::warn("[{}] serialize failed: {}", log_tag, status.ToString());
    return false;
  }
  return true;
}

/// @brief 从 JSON 字符串解析 protobuf Message
template <typename MessageT>
  requires std::is_base_of_v<google::protobuf::Message, MessageT>
[[nodiscard]] bool ProtoFromJson(const std::string& json,
                                 MessageT& message,
                                 const ProtoJsonOptions& options = {},
                                 const std::string_view log_tag = "ProtoJson") {
  message.Clear();
  ...
  if (!status.ok()) {
    spdlog::warn("[{}] parse failed: {}", log_tag, status.ToString());
    return false;
  }
  return true;
}
