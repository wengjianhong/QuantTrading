...
double AddAveragePrice(double current_average,
  ...
  return total_volume > 0
           ? (current_average * static_cast<double>(current_volume) + trade_price * static_cast<double>(trade_volume)) /
...
void PositionManager::ApplyPositionSnapshot(const std::vector<qtrade_sdk::trader::Position>& positions) {
  ...
  for (const auto& source : positions) {
    ...
    const auto yesterday = std::clamp(source.yesterday_volume, std::int64_t{0}, source.total_volume);
...
void PositionManager::ApplyTrade(const Trade& trade) {
  ...
  const bool is_open = trade.position_effect == qtrade_sdk::trader::PositionEffectType::kOpen;
  ...
  if (is_open || (is_cash && trade.side == qtrade_sdk::trader::SideType::kBuy)) {
    if (trade.side == qtrade_sdk::trader::SideType::kBuy) {
      ...
      position.long_avg_price = AddAveragePrice(position.long_avg_price, current, trade.price, trade.volume);
      ...
      position.short_avg_price = AddAveragePrice(position.short_avg_price, current, trade.price, trade.volume);
  ...
  if (trade.side == qtrade_sdk::trader::SideType::kSell) {
    ReducePosition(position.long_today, position.long_yesterday, trade.position_effect, trade.volume);
    if (LongTotal(position) == 0) {
      position.long_avg_price = 0.0;
    }
  } else if (trade.side == qtrade_sdk::trader::SideType::kBuy) {
    ReducePosition(position.short_today, position.short_yesterday, trade.position_effect, trade.volume);
