...
ErrorCode ComplianceManager::Configure(const ComplianceRules& rules) {
  // 1. 校验规则数值范围
  if (rules.min_volume <= 0 || rules.max_volume < rules.min_volume ||
      rules.min_price < 0.0 || rules.max_price < 0.0 ||
      (rules.max_price > 0.0 && rules.max_price < rules.min_price) ||
      rules.max_notional < 0.0) {
...
ErrorCode ComplianceManager::CheckOrder(const qtrade_sdk::trader::OrderRequest& request) const {
  ...
  if (request.volume < rules.min_volume || request.volume > rules.max_volume ||
      ...
      (rules.max_price > 0.0 && request.price > rules.max_price) ||
      !std::isfinite(notional) ||
      (rules.max_notional > 0.0 && notional > rules.max_notional) ||
      (!rules.allowed_instruments.empty() &&
       !rules.allowed_instruments.contains(request.instrument)) ||
      (!rules.allowed_sides.empty() && !rules.allowed_sides.contains(request.side)) ||
      (!rules.allowed_price_types.empty() &&
       !rules.allowed_price_types.contains(request.price_type))) {
    return ErrorCode::kSystemError;
  }
  return ErrorCode::kSuccess;
}
