...
/// @brief     单实例写入围栏与 engine_epoch 分配
/// @details   通过共享路径上的排他 flock 保证同一账户仅一个引擎进程可写；
///            持有围栏期间在文件中写入单调递增的 engine_epoch
/// @author    wengjianhong
/// @date      2026-07-16
...
class EngineFence {
 public:
  ...
  /// @brief 获取排他围栏并分配新 epoch
  /// @param path 围栏文件路径；多实例必须指向同一共享文件
  ...
  /// @brief 释放排他锁并关闭围栏文件描述符
  void Release();

  /// @brief 查询本次分配的 engine_epoch
  /// @return 持有围栏时返回已写入的 epoch；未持有时返回 0
